...
export const SubMenuOne: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setNewMenu(amenu)
    console.log(amenu)
    findTreeNode(amenu, "/menu-one/sub-menu-one").then((node: any) => {
      console.log(node)
      setValue(node?.name || "")
    })
  }, [])
  ...
  return (
    <>
      ...
      <Button type="primary" onClick={() => { changeMenu(changeTreeNode(newMenu, "/menu-one/sub-menu-one", value)) }}>确定</Button>
    </>
